...
def check_update():
    ...
    print("Current local version: " + localVersion, end=' ')
...
#        self.typeLink = Tkinter.Entry(frmDirectory)
#        self.typeLink.pack(side=Tkinter.RIGHT)
#        self.btnChoose = Tkinter.Button(frmDirectory, text="Ok", command=self.chooseLink)
#        self.btnChoose.pack(side=Tkinter.RIGHT)
        ...
        self.btnGenerate = Tkinter.Button(frmGenerate,
            width=10000,
        ...
        self.progress = 0
        self.progressbar = ttk.Progressbar(frmPgbar,
            orient="horizontal",
            mode="determinate",
            value=self.progress,
            maximum=1)
        ...
        """ QUIT """
        frmBottom = Tkinter.Frame(master)
        frmBottom.pack(fill=Tkinter.BOTH)

        self.btnOpenFolder = Tkinter.Button(frmBottom,
            width=15,
            text="Open hidx folder",
            state="disabled",
            command=self.openFolder)
        self.btnQuit = Tkinter.Button(frmBottom,
            width=15,
            text="QUIT",
    ...
    def chooseLink(self):
        selectedDirectory = self.typeLink.get()
        if len(selectedDirectory) > 1:
            self.lblSelected.config(fg="Black")
            self.lblDirectory.config(fg="Black")
            self.directory.set(selectedDirectory)
            self.btnGenerate.config(state="normal")
            self.typeLink.delete(0, 'end')
